...
class M3u8Download:
    ...
    def __init__(self, url, name, save_path, max_workers=64, num_retries=5, base64_key=None):
        self._url = url
        # self._name = name
        ...
        self._file_path = os.path.join(save_path, self._name)
    ...
    def output_mp4(self):
        ...
        cmd = 'ffmpeg -allowed_extensions ALL -i "%s.m3u8" -acodec copy -vcodec copy -f mp4 "%s.mp4"' % (
            self._file_path, os.path.join(os.path.dirname(self._file_path), self._name))
        # os.system(cmd)
        print(cmd)
        self.shell_run_cmd_block(cmd)
        try:
            os.rename(self._file_path + '.mp4', os.path.join(os.path.dirname(self._file_path), self.title) + '.mp4')
        except Exception as e:
            print(f'重命名错误：{e}')
            pass
...
# M3u8Download("https://cdn3.jiuse.cloud/hls/852527/index.m3u8?t=1690371365&m=xZbP2Q8NK2Z5qBuiU8UvYA", '你好')
